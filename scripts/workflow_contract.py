...
def validate(workflow):
    ...
    if len(re.findall(r'^      image: (\S+)$', workflow, re.MULTILINE)) != 1:
        failures.append('declare one container image')
    elif re.findall(r'^      image: (\S+)$', workflow, re.MULTILINE)[0] != CONTAINER_IMAGE:
        failures.append('use the reviewed digest-pinned Python 2.7.18 image')
